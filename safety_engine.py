...
class SafetyEngine:
    ...
    def __init__(
        ...
        """
        Initialize predictive safety engine.

        Args:
            tractor_center (tuple): Tractor position in ground plane (x, z), z=0 at tractor.
            homography (HomographyTransformer): For accurate ground plane transformation.
            storage (TrajectoryStorage): For velocity computation.
            image_size (tuple): (width, height) for normalization.
            prediction_frames (int): Number of frames for future path prediction.
            safety_zone_fraction (float): Fraction of image height defining safety zone.
            critical_ttc_threshold (int): Time-to-collision threshold for critical logging.
            critical_events_path (str): Path for storing critical safety event logs.
        """
        ...
        try:
            ...
            logging.warning(f"Could not create critical events directory: {e}")
